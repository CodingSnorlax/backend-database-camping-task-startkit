...
-- 3-2. 新增：承1，為三名教練新增專長資料至 `COACH_LINK_SKILL` ，資料需求如下：
  ...
  --  (select id from "USER" where email = 'lee2000@hexschooltest.io') 李燕容
  --  (select id from "USER" where email = 'muscle@hexschooltest.io') 肌肉棒子
   -- (select id from "USER" where email = 'starplatinum@hexschooltest.io') Q太郎
...
delete from "SKILL" where id = '4853017b-b684-49f1-8b87-9173fd9f3e65';
...
-- 5-3. 新增：`王小明`再次預約 `李燕容`   的課程，請在`COURSE_BOOKING`新增一筆資料：
    ...
    -- 3. 狀態`status` 設定為即將授課
insert into "COURSE_BOOKING" (course_id, user_id, booking_at, status) values
((select id from "COURSE" where user_id = (select id from "USER" where email = 'lee2000@hexschooltest.io')), (select id from "USER" where email = 'wXlTq@hexschooltest.io'), '2024-11-24 17:10:25', '即將授課');
...
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io') and cancelled_at is not null;
...
,d.name as 專長名稱
from "USER" as a --取姓名
inner join "COACH" as b on a.id = b.user_id --取經驗年數
inner join "COACH_LINK_SKILL" as c on b.id = c.coach_id --取skill_id
inner join "SKILL" as d on d.id = c.skill_id 
where d.name = '重訓'
